...
**Acceso (OB-7 / CEO-64, 2026-08-14):** Login **Google + autorización del panel**. Tras el OAuth, el callback consulta por SQL directo `SELECT permitido, nombre, rol FROM panel.acceso_app(email,'obleas')` (función del panel en `cdp_nova`, fuente única de la regla de acceso). Entra solo si `permitido=true` (usuario `activo` con la sección `obleas`, o `super_admin` como Ariel). **Ya NO se auto-crea usuario**, **NO hay clave de emergencia** (`APP_CLAVE` eliminado) y es **fail-closed** (si la DB/panel no responde → rechazo). El **dominio** (`ALLOWED_DOMAINS = novagnc.com.ar, sorvicor.com.ar`) quedó solo como defensa en profundidad (descarta gmail personal antes de consultar) — ya NO decide quién entra. Para dar acceso a alguien: tildarle la sección `obleas` en el panel (`usuarios.html`). Casos: Vanesa Urquia entra; Nahir Alarcon (sin la sección) es rechazada con mensaje claro.

> **Ojo con el alta de usuarios (verificado 2026-08-25).** Obleas no auto-crea, pero **el panel sí**: su callback de Google (`panel-nova/routes/auth.js:112-118`) da de alta sola a cualquier cuenta de los dominios permitidos, con rol `coord_ventas` y secciones `{dashboard,faq}` — y ese INSERT **no queda en `panel.audit_log`**, a diferencia de las altas del CRUD. Así apareció `vane@sorvicor.com.ar` sin que Ariel lo hubiera creado. Consecuencia práctica para obleas: **un usuario puede existir en `panel.usuarios` sin que nadie lo haya dado de alta a mano**; no entra a obleas hasta que alguien le tilde la sección, pero al diagnosticar "¿quién es este usuario?" no alcanza con mirar la auditoría. Encargo abierto para que nazca sin acceso: **PN-125**.
> **Y la cuenta real de Vanesa es `vane@sorvicor.com.ar`, no `vurquia@`** — el alta manual del 14-08 usó un mail que ella no usa (`ultimo_login` sigue en null); entró por primera vez el 25-08 con `vane@`. Quedaron los dos registros: el perfil completo (`coord_obleas` + organigrama/cobranzas/sorvicor_crpc) está en el que no usa.

> **Cookie de sesión = `ob_sesion`, nunca `token` (OB-11, 2026-09-23).** Las cookies son por host **sin puerto**: Obleas (`192.168.0.18:3080`) y el Panel (`:3090`) compartían el casillero `token` y la última en escribir deslogueaba a la otra ("No pudimos abrir el panel con la sesión guardada"). Se renombró en `server.js` (Set-Cookie + `extraerToken`) y en `index/historial/informe/calidad-contactos.html`, con la regex **anclada** (`(?:^|;\s*)ob_sesion=`) para no matchear cookies ajenas que terminen igual. El logout borra **solo** `ob_sesion`: **no tocar nunca la cookie `token`, es del panel.** El `localStorage.token` queda como está (localStorage sí distingue puerto). Precedente: `administracion-grupo` ADM-10 (`adm_sesion`).
