...
function clasificarPatente(registro, apiResp, config) {
  ...
  // Comparar por fechaVencimiento de la nueva oblea (cubre renovaciones anticipadas).
  // Si la oblea actual vence DESPUÉS del período analizado → renovó, sin importar cuándo lo hizo.
  // Fallback a fechaHabilitacion si no hay fechaVencimiento disponible.
  const nuevoVtoDate = data.datosOperacion.fechaVencimiento ? new Date(data.datosOperacion.fechaVencimiento) : null;
  const fechaOpDate = data.datosOperacion.fechaHabilitacion ? new Date(data.datosOperacion.fechaHabilitacion) : null;
  const esNuevaOp = vtoOrigDate && (
    (nuevoVtoDate && nuevoVtoDate > vtoOrigDate) ||
    (!nuevoVtoDate && fechaOpDate && fechaOpDate >= vtoOrigDate)
  );
...
async function consultarPatente(dominio, config) {
  const forceParam = config.force ? '&force=1' : '';
  const url = `${config.apiUrl}/api/consulta?patente=${encodeURIComponent(dominio)}&formato=enargas${forceParam}`;
