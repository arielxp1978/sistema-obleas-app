...
// Talleres propios Nova (lista fija). CORREGIDO 2026-08-18 (encargo OB-8): antes decía
// QUT0867 = Grupo P5, y estaba al revés. Verificado contra la razón social que ENARGAS trae
// en los datos crudos (`nova_operaciones.datos_raw->>'TRAZSOC'`, DB enargas_data):
//   IRT0550 = SORVICOR SRL      → Nova Gral Paz  ✅
//   HIT0797 = NOVA GNC SRL      → Nova R20       ✅
//   QUT0856 = GRUPO P5 S.R.L.   → Grupo P5       ✅ (opera desde 12/2025; por eso no estaba
//                                                    en los exports de 2026-07 que se miraron)
//   QUT0867 = CAR EQUIP S.A.S.  → AJENO          ❌ (7.221 ops de un tercero que se contaban
//                                                    como propias; misma empresa que HIT0714)
//   QUT0865 = sin razón social  → AJENO          ❌ (714 ops, 2023-09 a 2024-11)
// Corroborado además por COMISIONISTAS_PROPIOS más abajo: los canales de Grupo P5 son `856@*`
// (el comisionista se prefija con el número del taller) y no existe ningún `867@*`.
const TALLERES_PROPIOS = new Set(['IRT0550', 'HIT0797', 'QUT0856']);
...
const COMISIONISTAS_PROPIOS = new Set([
  ...
  '797@2', '797@3', '797@4', '797@5', '797@11',   // canales internos Nova R20 (HIT0797) — 797@11 = PROMO TP (confirmado Ariel 2026-07-29)
...
const CANAL_NOMBRES = {
  ...
  '797@2': 'Nova R20 interno', '797@3': 'Nova R20 interno', '797@4': 'Nova R20 interno', '797@5': 'Nova R20 interno', '797@11': 'PROMO TP',
...
// Nombre legible del comisionista de una fila. Prioridad: el nombre que trae el propio dato
// (subtaller_nombre de InfoSys, mapeado a COMISIONISTA_NOMBRE) → el mapa de canales propios
// conocidos → vacío (el front cae al código). El CSV no trae nombre, así que ahí queda el código.
function nombreComisionista(r) {
  const nom = (r.COMISIONISTA_NOMBRE || '').trim();
  if (nom) return nom;
  return CANAL_NOMBRES[getComisionista(r)] || '';
}
...
function procesarTelefonos(rows) {
  return rows.map((r, idx) => {
    ...
    return {
      ...
      UTELEFONO: n.limpio,
      // Sugerencia: número válido si está OK; si es LEVE al que solo le falta el área, se completa
      // con Córdoba (351) — ver sugerirCordoba() en normalizar.js. RECHAZAR y "falta un dígito" → vacía.
      UTELEFONO_SUGERENCIA: n.sugerencia || '',
...
function generarMetricas(rows, titulo) {
  const m = { titulo, total: rows.length, por_tcodtal: {}, por_comisionista: {}, nombresComisionista: {}, telefonos: { total: rows.length, sin: 0, mal: 0, ok: 0, detalle: {} }, por_usuario: {} };
  const users = {};
  rows.forEach(r => {
    const t = r.TCODTAL || ''; m.por_tcodtal[t] = (m.por_tcodtal[t] || 0) + 1;
    // Distribución por comisionista (GNCOBS3/SUBTAL) — para el gráfico "por Comisionista" del Original.
    const com = getComisionista(r) || '(Sin comisionista)'; m.por_comisionista[com] = (m.por_comisionista[com] || 0) + 1;
    // Nombre legible del comisionista (para mostrar el nombre en vez del código en gráficos/banner).
    if (com !== '(Sin comisionista)' && !m.nombresComisionista[com]) {
      const nom = nombreComisionista(r); if (nom) m.nombresComisionista[com] = nom;
    }
...
// Prioriza `_tipoGestion` precalculado: ese campo SÍ sobrevive el round-trip por mapRegistro (que
// no reexpone UCODGEST). Sin esto, al reenviar STATE.registros a /api/generar-archivos (al cambiar
// "teléfonos por archivo") UCODGEST venía vacío → todo se reclasificaba como oblea y los archivos
// PH desaparecían para siempre. UCODGEST queda como fallback para las filas crudas del CSV/base.
// (fix 2026-08-14)
function tipoGestion(r) {
  if (r && r._tipoGestion) return String(r._tipoGestion).toLowerCase() === 'ph' ? 'ph' : 'oblea';
...
function dividirArchivos(rows, opts = {}) {
  const porArchivo = Math.max(1, parseInt(opts.porArchivo) || 100);
...
module.exports = { parseCSV, filtrar, esTallerNuestro, esComisionistaPropio, TALLERES_PROPIOS, COMISIONISTAS_PROPIOS, canalLabel, tipoGestion, telWhatsappValido, procesarTelefonos, generarMetricas, dividirArchivos, procesarCompleto, procesarRows, normalizarLocalidad, KEEP_COLS };
