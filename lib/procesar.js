...
// Talleres propios Nova. QUT0856 = R20/Grupo P5 (el código real que trae la megatabla);
// QUT0865 se mantiene por compatibilidad histórica. IRT0550 solo si no tiene comisionista
// propio (GNCOBS3 vacío). Debe coincidir con NOVA_TALLERES de server.js.
function esTallerNuestro(r) {
  const t = (r.TCODTAL || '').trim(), g = (r.GNCOBS3 || '').trim();
  return (t === 'IRT0550' && g === '') || t === 'HIT0797' || t === 'QUT0856' || t === 'QUT0865';
}

// Comisionistas propios Nova (canales internos). El campo SUBTAL tiene formato
// `<taller>@<codigo>`. Estos son NUESTROS canales; cualquier otro SUBTAL con `@`
// es un taller externo que opera con nuestro PEC → se excluye del broadcast a ManyChat.
// OJO: la regla "1-2 dígitos = interno" NO sirve (Bronte 550@43 es externo con 2 díg.),
// por eso es una lista explícita. SUBTAL vacío / sin `@` = trabajo directo = nuestro.
const COMISIONISTAS_PROPIOS = new Set([
  '550@5', '550@6', '550@15',   // PROMOTP (online obleas/PH)
  '856@11',                     // PROMO TP (online)
  '856@4',                      // Mostrador
  '856@2'                       // Agencia (autos 0km)
]);
