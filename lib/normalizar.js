...
// Sugerencia "completar Córdoba": si al número solo le falta el código de área y es un abonado
// local de 7 díg (o un celular con prefijo 15 → abonado de 7), se antepone 351 (Córdoba capital)
// y queda un número válido de 10 díg. Si le falta un dígito o es otra cosa, NO se sugiere (no se
// inventa). Decisión con Ariel 2026-07-29: la mayoría de los clientes son de Córdoba y Yhonny
// revisa antes de exportar.
function sugerirCordoba(local) {
  let s = (local || '').toString().replace(/\D/g, '');
  if (s.length === 9 && s.startsWith('15')) s = s.slice(2);  // celular local: sacar el "15"
  if (s.length === 7) return '351' + s;                       // abonado Córdoba capital → +351
  return '';
}

function normalizarTel(raw) {
  const rej = (d, tipo, desc) => ({ limpio: d, whatsapp: '', estado: 'RECHAZAR', errorTipo: tipo, errorDesc: desc, sugerencia: '' });
  const lev = (d, tipo, desc) => ({ limpio: d, whatsapp: '', estado: 'LEVE', errorTipo: tipo, errorDesc: desc, sugerencia: sugerirCordoba(d) });
  const ok = (d) => ({ limpio: d, whatsapp: '549' + d, estado: 'OK', errorTipo: 'NINGUNO', errorDesc: '', sugerencia: d });

  if (!raw) return rej('', 'SIN_TELEFONO', 'Sin número');
  let d = raw.toString().replace(/[^\d]/g, '');
  if (!d || /^0+$/.test(d)) return rej(d, 'SIN_TELEFONO', 'Sin número');   // '0', '000000', etc = sin teléfono
...
function analizarTelError(raw) {
  ...
  if (!d || /^0+$/.test(d)) return 'SIN_TELEFONO';   // '0', '000000', etc = sin teléfono, no "repetido"
