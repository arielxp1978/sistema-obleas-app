...
/**
 * Reemplaza SOLO la verificación de un período ya guardado (la usa la actualización
 * automática). No toca guardadoEn ni version: esos dicen cuándo lo guardó una persona.
 * Escritura atómica (tmp + rename) para no dejar un JSON a medias si algo se corta.
 */
function actualizarVerificacion(periodoId, verificacion) {
  const filepath = path.join(DATA_DIR, `${periodoId}.json`);
  const actual = leerPeriodo(periodoId);
  if (!actual) throw new Error('período no encontrado');
  const tmp = filepath + '.tmp';
  fs.writeFileSync(tmp, JSON.stringify({ ...actual, verificacion }, null, 2), 'utf8');
...
module.exports = { actualizarVerificacion, guardarPeriodo, leerPeriodo, listarPeriodos, eliminarPeriodo, generarHistorial };
